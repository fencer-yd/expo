      ...
      describeManifestTypes(
        ...
        it(`creates a redirect URL`, () => {
          ...
          if (manifestObj.manifest) {
            ...
            // TODO(wschurman): for some reason this has a slash on the end for new manifests
            // which don't ever have hostUri in them
            expect(makeRedirectUri()).toBe('exp://192.168.1.4:19000/');
          }
        });
        ...
        it(`creates a redirect URL with a custom path`, () => {
          ...
          if (manifestObj.manifest) {
            ...
            // TODO(wschurman): for some reason this has a slash on the end (making a double slash) for new manifests
            // which don't ever have hostUri in them
            expect(makeRedirectUri({ path: 'bacon' })).toBe('exp://192.168.1.4:19000//--/bacon');
          }
        });
